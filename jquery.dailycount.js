...
{
    /*
     * jQuery dailycount 0.2 ($Rev: 2 $)
     * Copyright (c) 2013 Krishna Pennacchioni (krishna@agentelinux.com.br)
     * Changed: removed the hour, minute and second the calculation of days between dates.
     */
    /*
     * Source form jQuery EasyDate 0.2.4 ($Rev: 54 $)
    ...
    var defaults = {
        ...
        locale: $.dailycount.locales.enUS
    };
    ...
    var pause = $.dailycount.pause = function(selector)
    {
    ...
    var resume = $.dailycount.resume = function(selector)
    {
    ...
    var set_now = $.dailycount.set_now = function(date)
    {
        var time;
    ...
    var get_now = $.dailycount.get_now = function()
    {
        var now = new Date();
        ...
        var now_new = new Date(curr_year,curr_month,curr_date);
        now.setTime(now_new.getTime() + now_diff);

        return now;
    };
    
    // Formats a Date object to a human-readable localized string.
    var format_date = $.dailycount.format_date = function(date, options)
    {
        ...
        var post_new = new Date(post_year,post_month,post_date);
        
        var diff = ((get_now().getTime() - post_new.getTime()) / 1000);
    ...
    //     1) element.data("dailycount.date") (in case we previously cached it)
    ...
    {
        var date = element.data("dailycount.date");
        ...
        if(isNaN(date))
        {
            ...
            if(!isNaN(time))
            {
                date = new Date();
                date.setTime(time);
                element.data("dailycount.date", date);
    ...
    {
        var settings = element.data("dailycount.settings");
    ...
    $.fn.dailycount = function(options)
    {
        var settings = $.extend({}, defaults, options);
        this.data("dailycount.settings", settings);
        
        // Clear any cached dates in case the timestamp has been updated since
        // the last dailycount() call on any of these elements.
        this.removeData("dailycount.date");
        
        this.each(function()
        {
            // Make sure that we aren't updating the element multiple times in
            // case dailycount() was called on the same element more than once.
